use crate::{
  error::Error,
  raw::{DiskMmapFile, DiskMmapFileMut},
  MmapFile, MmapFileMut,
};
...
impl Options {
  ...
  ///
  pub unsafe fn create_mmap_file_mut<P: AsRef<Path>>(self, path: P) -> Result<MmapFileMut, Error> {
  ...
  /// See the [crate-level safety section](crate) for the full contract.
  ///
  pub unsafe fn open_mmap_file<P: AsRef<Path>>(self, path: P) -> Result<MmapFile, Error> {
  ...
  /// See the [crate-level safety section](crate) for the full contract.
  ///
  pub unsafe fn open_exec_mmap_file<P: AsRef<Path>>(self, path: P) -> Result<MmapFile, Error> {
  ...
  /// See the [crate-level safety section](crate) for the full contract.
  ///
  pub unsafe fn open_mmap_file_mut<P: AsRef<Path>>(self, path: P) -> Result<MmapFileMut, Error> {
  ...
  /// See the [crate-level safety section](crate) for the full contract.
  ///
  pub unsafe fn open_exist_mmap_file_mut<P: AsRef<Path>>(
    self,
    path: P,
  ...
  /// See the [crate-level safety section](crate) for the full contract.
  ///
  pub unsafe fn open_cow_mmap_file_mut<P: AsRef<Path>>(
    self,
    path: P,
  ) -> Result<MmapFileMut, Error> {
...
mod tests {
  ...
  fn test_open_mmap_file() {
    ...
    unsafe {
      ...
      file
        .write_all("some data...".as_bytes(), "sanity text".len())
        .unwrap();
      file.flush().unwrap();
      drop(file);
  ...
  fn test_open_mmap_file_mut() {
    ...
    unsafe {
      ...
      // modify the file data
      file
        .truncate(("some modified data...".len() + "sanity text".len()) as u64)
        .unwrap();
      file
        .write_all("some modified data...".as_bytes(), 0)
        .unwrap();
      file.flush().unwrap();
      drop(file);
      ...
      // skip the sanity text
      file
        .read_exact(buf.as_mut_slice(), "sanity text".len())
        .unwrap();
      assert_eq!(buf.as_slice(), "some modified data...".as_bytes());
    }
  }
