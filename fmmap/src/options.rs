macro_rules! declare_and_impl_options {
  ($name: ident, $file_open_options: ident) => {
    ...
    pub struct $name {
      ...
      /// Whether the user requested truncate(true). Stored separately from
      /// `file_opts` so the `open` step is non-destructive: we acquire the
      /// auto-lock first, then apply this via `set_len(0)`. This way, a
      /// lock-contended open doesn't destroy the existing file content.
      pub(crate) truncate_flag: bool,
      /// User-requested mapping offset. Mirrored in `mmap_opts.offset()`;
      /// duplicated here because `MmapOptions` doesn't expose a getter and
      /// `truncate` needs it to clamp the new mapping against the new EOF.
      pub(crate) offset: u64,
      /// User-requested mapping length. Mirrored in `mmap_opts.len()`. `None`
    ...
    impl $name {
      ...
      ///
      /// This field is **always ignored** by every mmap opener in this crate.
      /// The file handle is only used to back the mapping, never written to
      /// directly, so append-mode access has no effect. On Windows, requesting
      /// append-mode would actually be harmful: it strips `FILE_WRITE_DATA`
      /// from the access mask and breaks writable mappings, so the writable
      /// openers force it off.
      ///
      ...
      /// Request that the file be truncated to length 0 after it is opened.
      ///
      /// This is logically equivalent to [`std::fs::OpenOptions::truncate`],
      /// but the truncation is performed by `fmmap` itself **after** the
      /// auto-acquired file lock has been obtained — so a lock-contended
      /// open will return an error with the existing file contents intact,
      /// rather than destroying them as a passthrough to `OpenOptions::truncate`
      /// would.
      ///
      /// This field is ignored when opening [`DiskMmapFile`], [`AsyncDiskMmapFile`], [`MmapFile`] and [`AsyncMmapFile`].
      ///
  ...
  macro_rules! declare_and_impl_async_options {
    ($filename_prefix: literal, $doc_test_runtime: literal, $path_str: literal) => {
      ...
      impl AsyncOptions {
        ...
        pub async unsafe fn create_mmap_file_mut<P: AsRef<Path>>(self, path: P) -> Result<AsyncMmapFileMut, Error> {
        ...
        #[doc = ""]
        #[doc = "See the [crate-level safety section](crate) for the full contract."]
        pub async unsafe fn open_mmap_file<P: AsRef<Path>>(self, path: P) -> Result<AsyncMmapFile, Error> {
        ...
        #[doc = ""]
        #[doc = "See the [crate-level safety section](crate) for the full contract."]
        pub async unsafe fn open_exec_mmap_file<P: AsRef<Path>>(self, path: P) -> Result<AsyncMmapFile, Error> {
        ...
        #[doc = ""]
        #[doc = "See the [crate-level safety section](crate) for the full contract."]
        pub async unsafe fn open_mmap_file_mut<P: AsRef<Path>>(self, path: P) -> Result<AsyncMmapFileMut, Error> {
        ...
        #[doc = ""]
        #[doc = "See the [crate-level safety section](crate) for the full contract."]
        pub async unsafe fn open_exist_mmap_file_mut<P: AsRef<Path>>(self, path: P) -> Result<AsyncMmapFileMut, Error> {
        ...
        #[doc = ""]
        #[doc = "See the [crate-level safety section](crate) for the full contract."]
        pub async unsafe fn open_cow_mmap_file_mut<P: AsRef<Path>>(self, path: P) -> Result<AsyncMmapFileMut, Error> {
  ...
  macro_rules! impl_async_options_tests {
    ($filename_prefix: literal, $runtime: meta, $path_str: ident) => {
      ...
      mod tests {
        ...
        // entitlements. `cargo test` binaries don't qualify, so skip there.
        #[$runtime]
